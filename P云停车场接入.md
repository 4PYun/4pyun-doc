...
| 订单支付结果通知  |   完成支付后，通过该接口完成订单同步.   |   Y     | 支付结果通知    |
| 停车场实时数据拉取       |   获取停场的车辆数和总车位等数据  |  Y  |
| 最近出入场车辆查询        |   P云会按照一定的时间频率向停车场拉取停车信息  |  Y  |
...
| locking_status | short | Y | 锁车标识: 1已锁, 0未锁, -1不支持 |
| autopay_status | int | N | 自动支付状态: 1已开启, 0未开启 |
...
| parking_order | string | Y | 停车支付订单号, 原客户端提交. |
...
P云将固定频率调用该接口完成数据获取, 高峰时段每3分钟获取一次, 非高峰时段30分钟获取一次.每次查询需返回时间范围内的所有出/入场车辆信息.
...
| 字段 | 类型 | 必须  | 说明|
| --- | --- | --- | --- |
| service | string | Y | 服务名: `service.parking.records` |
| version | string | Y | 版本号:`1.0`|
| charset | string | Y | 字符集:`UTF-8`|
| sign | string | Y | 签名|
...
| --- | --- | --- | --- |
| park_uuid | string | Y | 停车场编号 |
| start_time | string | Y | 查询范围开始时间, 格式:`yyyyMMddHHmmss`. |
| end_time | String | Y | 查询范围结束时间, 格式:`yyyyMMddHHmmss`. |
...
| 字段 | 类型 | 必须  | 说明|
| --- | --- | --- | --- |
| devices | array | N | 设备状态集合, 对象参考后面的设备状态. |
| enters | array | N | 入场车辆集合, 对象参考后面车辆信息. |
| leaves | array | N | 出场车辆集合, 对象参考后面车辆信息. |

**车辆信息:**

| 字段 | 类型 | 必须  | 说明|
| --- | --- | --- | --- |
| plate | string | N | 对于支持车牌识别的停车场应返回. |
| plate_color | short | N | 车牌颜色<br>1 蓝色<br>2 黄色<br>3 白色<br>4 黑色<br>5 绿色<br>-1 未知 |
...
| prepaid | int | N | 已开启自动支付权限:1启用,0未开启 |
...
| total_value | int | Y | 当前总停车费用(单位分) |
| free_value | int | Y | 已优惠金额(单位分) |
| online_value | int | N | 线上收费金额(单位分) |
| cash_value | int | N | 现金收费金额(单位分) |
| prepaid_value | int | N | 自动支付金额(单位分) |
...
注:`total_value = online_value + cash_value + prepaid_value + free_value`
...
| --- | --- | --- | --- |
| name | string | Y | 设备名称 |
| type | short | Y | 设备类型:<br/>1: 摄像头<br/>2: 道闸<br/>3: PC<br/>4: 数据库 |
| status | short | Y | 状态:<br/>1: 正常<br/>0: 异常<br/>-1: 未知 |
...
| --- | --- | --- | --- |
| park_uuid | string | Y | 停车场编号 |
| passport | string | Y | 用户通行证ID, 停车场可用作虚拟卡ID, 出场将传入, 例如: 无A123456@4pyun.com |
...
| type           | int | Y  | 优惠类型: 1金额, 2时长, 3全免, 4不同计价券, 5长期免费券 |
| value          | int | Y  | 优惠金额:<br/>当`type=1`时单位为分;<br/>当`type=2`时单位为分钟;<br/>当`type=3`时暂无定义;<br/>当`type=4`时为停车场计价规则ID;<br/>当`type=5`时为在派发后单位小时后重复进出均可免费停车.|
...
P云发起向停车场, 查询指定车辆月卡/次卡/储值卡信息, 停车场返回对应车牌/客户信息的所有有效月卡/次卡/储值卡办理信息.
...
| version | string | Y | 版本号:`1.0`|
| charset | string | Y | 字符集:`UTF-8`|
| result_code | string | Y | 本次请求状态返回码, 示例:1001<br/>状态码  含义<br/>1001  接口处理成功, 业务参数将返回.<br/>1002  没有查到相关贵宾记录.<br/>1401  签名错误, 请检查配置.<br/>1500  接口处理异常. |
| sign | string | Y | 当前请求应答结果, 签名方法参考附录. |
| message | string | Y | 状态码处理描述, 如:返回错误信息. |

| 字段 | 类型 | 必须  | 说明|
| --- | --- | --- | --- |
| vips | array | N | 贵宾集合, 即月卡/次卡/储值卡中的一种, 对象参考后面贵宾信息.|
...
| charge_desc | string | N | 当前贵宾收费标准描述;例如: 月卡B.                                                    |
...
| type| int | Y | 贵宾类型:<br/>1 月卡<br/>2 储值<br/>3 储次<br/>4 储天<br/>5 年卡<br/>6 季度卡<br/>7 半年卡<br/>0 免费停车 |
| balance  | int | Y | 当前余额<br/>储值: 为余额, 单位分;<br/>储次:为剩余次数;<br/>储天:为剩余天数;<br/>月卡/年卡/半年卡/季度卡:为剩余天数.|

<font color='red'>注: 若月卡当前到期则balance=0, 按照此规则计算</font>

### 6.2 车辆续费通知

**协议说明:**
P云发起请求, 对月卡, 次卡或储值卡进行续费.
